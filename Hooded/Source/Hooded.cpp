...
const void Hooded::Move(Camera& camera, float deltaTime, MapManager& mapManager, sf::Sprite& sprite)
{
	...
	// Gravity
	if (!mapManager.SpriteOnGround(sf::Vector2f(position.x, position.y + m_gravity * m_speed * deltaTime),
		sprite.getTextureRect().getSize()))
	{
		sprite.setPosition(position.x, position.y + m_gravity * m_speed * deltaTime);
	}

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::A)
		&& !mapManager.MapCollision(sf::Vector2f(position.x - 1 * m_speed * deltaTime, position.y),
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::W)
		&& !mapManager.MapCollision(sf::Vector2f(position.x, position.y - 1 * m_speed * deltaTime),
			sprite.getTextureRect().getSize(), m_dematerialized)
		&& mapManager.SpriteOnGround(sf::Vector2f(position.x, position.y + m_gravity * m_speed * deltaTime),
			sprite.getTextureRect().getSize()))
	{
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::S)
		&& !mapManager.MapCollision(sf::Vector2f(position.x, position.y + 1 * m_speed * deltaTime),
			sprite.getTextureRect().getSize(), m_dematerialized))
	{
		sprite.setPosition(position.x, position.y + 1 * m_speed - m_speed * deltaTime);
	}
